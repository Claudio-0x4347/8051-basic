...
PASSOS		EQU	R5		; ARMAZENA OS NIBBLES DOS PASSOS
...
INICIO: 	MOV PAVIMENTO, #01	; COMEÇA COM O ELEVADOR NO PAVIMENTO 1
		MOV PASSOS, #33H	; INICIALIZA OS NIBBLES DOS PASSOS
...
UP:		LCALL ATRASO		; DELAY 5S
		...
		LCALL MOTOR_OFF		; DESLIGA MOTOR CC
		LCALL ABRIR		; ABRE O ELEVADOR
		LCALL ATRASO
		LCALL FECHAR		; FECHA O ELEVADOR
		RETI
...
; ============ SUBROTINA PARA GIRAR O MOTOR DE PASSO ================
ABRIR:		MOV DPTR, #2000H	; ENDEREÇO 2000H DO MAPEAMENTO DE MEMORIA
		MOV R2, #90		; 90 PASSOS(360/4) = 1 VOLTA
		MOV A, PASSOS		; RECUPERA OS VALORES DOS NIBBLES
		CPL A			; O NIBBLE ARMAZENADO ESTÁ SEMPRE ATRASADO 1 PASSO
STEP1:		MOVX @DPTR, A		; EFETUA O PASSO DE 4º
		RR A
		MOV R4, #80H		; ATRASO PARA MOVIMENTAÇÃO DO MOTOR
V1:		MOV R6, #0FFH
		DJNZ R6, $
		DJNZ R4, V1
		DJNZ R2, STEP1
		MOV PASSOS, A		; ATUALIZA O VALORES DOS NIBBLES
		RET
		
FECHAR:		MOV DPTR, #2000H	; ENDEREÇO 2000H DO MAPEAMENTO DE MEMORIA
		MOV R2, #90		; 90 PASSOS(360/4) = 1 VOLTA
		MOV A, PASSOS		; RECUPERA OS VALORES DOS NIBBLES 1 PASSO A FRENTE
		CPL A			; VOLTA 2 PASSOS(JA CONSIDERA 1 PASSO NO SENTIDO DE FECHAR)
STEP2:		MOVX @DPTR, A		; EFETUA O PASSO DE 4º
		RL A
		MOV R4, #80H		; ATRASO PARA MOVIMENTAÇÃO DO MOTOR
V2:		MOV R6, #0FFH
		DJNZ R6, $
		DJNZ R4, V2
		DJNZ R2, STEP2
		MOV PASSOS, A		; ATUALIZA OS VALORES DOS NIBBLES
		RET
